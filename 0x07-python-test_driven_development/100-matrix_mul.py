...
def matrix_mul(m_a, m_b):
    if not isinstance(m_a, list):
        raise Typerrror('m_a must be a list')
    ...
    if any(False in x for x in [[isinstance(ele, (int,float)) for ele in listx] for listx in m_a]):
        raise TypeError('m_a should contain only integers or floats')
    if any(False in x for x in [[isinstance(ele, (int, float)) for ele in listx] for listx in m_b]):
    ...
    if len(set(len(listx) for listx in m_b)) >1:
        raise TypeError('each row of m_b shoukd be of the same size')
